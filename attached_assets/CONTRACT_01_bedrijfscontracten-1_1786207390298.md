...
### 2.3 De bewaking — grotendeels al geregeld, één gat

**Correctie op de eerste versie van deze opdracht.** De bewaking draait al vanzelf. `lib/bewakingsloop.ts` (626 r.) wordt bij opstart gestart via `planDagelijkseBewakingsloop()` (`index.ts` r.83), en de voeder `voedFinancieleContracten()` (r.122) roept `voerFinancieleContractBewakingUit()` aan en zet de signalen op de werkbak. Er is dus **geen nieuwe planner nodig en geen aanhaking te bouwen**.

Wat er nog wél moet:

- **Controleer en meld wát `voerFinancieleContractBewakingUit()` precies signaleert.** Het commentaar in de loop noemt alleen "verlengt automatisch". Vereist zijn ook: naderende einddatum, indexeringsmaand, en vooral de **opzegdatum berekend uit einddatum minus opzegtermijn** — dat is de datum die er werkelijk toe doet. Een leasecontract dat 1 juni afloopt met drie maanden opzegtermijn moet op 1 maart afgaan; op de einddatum waarschuwen is te laat. Ontbreekt dat, dan wordt het toegevoegd.
- **De documentvervaldatums uit §2.2 als bron toevoegen** aan dezelfde loop, via `syncBron`. Geen nieuwe meldingentabel — de signaleringstabel bestaat al.
- **Ontvanger is de administratie**, bepaald op recht (`financieel` niveau 2 of hoger), niet op een naam of e-mailadres in de code. De loop stuurt deze bron nu naar René ("→ René (Weten)", r.121); meld of dat zo blijft of dat de administratie de eerste ontvanger wordt.
...
5. Het signaal komt aan bij de administratie zonder dat iemand een knop indrukt. Toon de regel uit `bewaking_draaien` van de draai die het opleverde.
