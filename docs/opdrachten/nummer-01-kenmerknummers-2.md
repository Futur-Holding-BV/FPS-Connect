...
### 4.10 Kopiëren en bevriezen (besluit René)

**Een calculatie of offerte die naar een klant is gestuurd wordt nooit
overschreven.** Wil je hem herzien, dan **kopieer** je hem: de kopie krijgt een
nieuw eigen volgnummer uit dezelfde reeks — `C601`, `O406` — en de verstuurde
versie blijft ongewijzigd bestaan.

Uitdrukkelijk **geen** letter-achtervoegsel zoals bij inkoop. Dat is bewust
asymmetrisch en dat is juist: een herziene inkoopopdracht **vervangt** de vorige
(de leverancier moet de nieuwe volgen), terwijl een herziene offerte **naast** de
vorige staat (beide bestaan, de klant kiest).

Vereisten:

- de kopie draagt een verwijzing naar het origineel (`gekopieerd_van_id`), zodat de
  samenhang zichtbaar is zonder dat het nummer die hoeft te dragen
- het moment van bevriezen is hetzelfde moment als in §4.3 en §4.6: **definitief
  maken / versturen**. Eén begrip, drie toepassingen — kenmerk vastvriezen,
  fiscaal factuurnummer toekennen, document onwijzigbaar maken
- een verstuurde offerte of calculatie is daarna alleen-lezen. Niet "afgeraden in de
  UI" maar geweigerd door de server

**Eerst meten, en dit is een echte valkuil.** Er bestaat al versiebeheer:
`offerte_versies` met `versienummer` (`schema/offertes.ts` r.176-179), routes
`GET`/`POST /offertes/:id/versies` (`routes/offertes.ts` r.1578-1626), plus
`voorwaarden_snapshot` en een gepind Document Studio-model die bij verzenden al
worden vastgelegd.

Stel vóór de bouw vast en meld: wordt er bij verzenden werkelijk een versie
weggeschreven, en wordt de offerte zelf daarna nog gewijzigd? **Bouw geen tweede
weg naast dit mechanisme.** Waarschijnlijk is de juiste uitkomst dat
`offerte_versies` het bevriezen doet en de kopieerfunctie de herziening — maar dat
moet blijken uit de code, niet uit deze aanname.
...
- wat `offerte_versies` vandaag werkelijk doet bij verzenden (§4.10)
