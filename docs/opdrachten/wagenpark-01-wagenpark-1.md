...
Jacqueline is beheerder van het wagenpark. Nu bestaat `GET /wagenpark/ai-advies` als lijst die je moet gaan ophalen — er wordt niets bezorgd.

1. **De bestaande signalen worden bezorgd in plaats van opgehaald**: APK, onderhoudsinterval, verzekering, lease, bandenwissel, en de nieuwe documentvervaldatums uit §2.
2. **Aansluiten op de werkbak uit `WERKBAK_01`** — geen eigen meldingenstroom voor het wagenpark erbij. Is `WERKBAK_01` nog niet gebouwd, dan wordt hier alleen de bron opgeleverd in de vorm die de werkbak verwacht, plus een e-mail aan de beheerder als tussenoplossing. **Geen nieuwe meldingentabel.**
3. **Ontvanger is de beheerder van het wagenpark**, niet een vast e-mailadres in de code. Er zit al een `toegewezen_beheerder_id` in het schema — gebruik dat, en is het leeg, val dan terug op wie recht `wagenpark` niveau 3 of hoger heeft.
4. **De sync moet vanzelf gaan.** `POST /wagenpark/sync` draait nu alleen als iemand hem aanroept. Hang hem aan dezelfde dagelijkse loop als de overige bewaking. Draait hij een etmaal niet, dan is dát een melding.
