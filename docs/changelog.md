...
Drie deelopdrachten uitgevoerd; niets aan prompts of AI-gedrag gewijzigd.

- **Poortwachter (toewijzingsgat dicht):** de AI-gateway weigert nu élke aanroep (chat én responses) zonder volledige logcontext — module, functie, promptNaam en promptVersie zijn alle vier verplicht (promptVersie nu ook type-verplicht). Weigering wordt gelogd als status "geweigerd" met de ontbrekende velden; er gaat dan géén verzoek naar het model. Alle 116 bestaande aanroepplekken bleken al compleet (volledige typecheck met verplicht promptVersie: 0 fouten). Bewijs: nieuwe unittest ai-gateway-poortwachter.test.ts — 5/5 groen.
- **Tokenraadsel (715k tokens) opgelost:** alle niet-gelabelde aanroepen dateren van vóór 10 aug 2026 (vóór de logcontext-verplichting). Grootverbruiker is de dagelijkse Scout-marktsignalenrun (gpt-4o, ~8–18k tokens per run, 60 RSS-artikelen per prompt; geen prompt-hash omdat die run alleen een user-message stuurt) — sinds 10 aug netjes gelabeld als scout/filterEnClassificeer. Meetscript meting-ai01-prod.ts uitgebreid met sectie "5. Attributie niet-gelabelde aanroepen" zodat René dit read-only op prod kan bevestigen. Bekend restpunt (gemeld, niet gewijzigd): het responses()-pad logt tokens/kosten nog als NULL/€0.
- **Generieke leerlus:** nieuwe route POST /api/ai/veld-correctie (whitelist op veld-prefix per scherm, zelfde tabel ai_veld_correcties en semantiek als de bestaande calculatie-/bedrijfsdocumentenleerlus: voorstel==gekozen ⇒ overgenomen, afwijkend ⇒ gecorrigeerd, leeg ⇒ afgewezen). Vastlegging ingebouwd (fire-and-forget, nooit blokkerend) in: centrale AI-invullen-knop (dekt 9 formulieren: CRM-organisatie/contact, gebouw, leverancier, werkmaatschappij, concurrent, voertuig, medewerker, magazijnartikel), spot-AI (web plattegrond én monteur-app), gereedschappen-foto-AI (nieuw+bewerken), gebouw-tekeningen-AI, AI-projectsamenvatting (gebouw-e-mails), offerte-e-mailvoorstel, SCAB-mail, HRM-dossiervoorstellen (accepteren/afwijzen/corrigeren), PIM-opdrachtvoorstel en Studio-huisstijlvoorstel. Monteur-app-incidenten-AI logt ook. Bewust niet aangesloten (geen concreet overnemen-moment, dus geen meetbare keuze): financieel contract-analyse (informatief) en de spot-afwerkflow in de monteur-app (AI stelt daar alleen toepassing voor). Hardening n.a.v. architect-review: migratie 0059 voegt gebruiker_id-audit toe aan ai_veld_correcties (herleidbaarheid tegen leerdata-vergiftiging), de route kent een volume-limiet (120/uur per gebruiker ⇒ 429), en HRM-/PIM-/Studio-logging is verplaatst naar ná de geslaagde echte mutatie (mislukte beoordeling logt niets). Belangrijk lek meteen gedicht: de bedrijfsdocumenten-few-shot (organisatie.ts) telde correcties over álle veld_namen — sinds de generieke leerlus zouden rijen van andere schermen (en verzonnen rijen) die prompt kunnen beïnvloeden; de drempel- en voorbeeldquery filteren nu hard op de zes bedrijfsdocument-velden. Generieke leerlus-rijen worden dus nergens als AI-leersignaal geconsumeerd — ze zijn nu uitsluitend meetdata. Restpunt (bewust, gemeld): wil je generieke correcties ooit wél als leersignaal gebruiken, dan is eerst server-side koppeling aan een uitgegeven AI-voorstel nodig (voorstel-tokens door alle AI-flows) — buiten scope van deze opdracht. Bewijs: scripts/src/verificatie-ai-veld-correctie.ts — 14/14 groen (authenticatie, whitelist-validatie, vastlegging overgenomen/gecorrigeerd/afgewezen, gebruiker_id-audit, rate-limit 429, en invariant dat gesmede generieke rijen buiten de bedrijfsdocumenten-few-shot blijven). Architect-review: PASS (na twee hardeningsrondes).
...
## 2026-08-17 — OTA-update bewaking: melding bij ontbrekend EXPO_TOKEN of eas-cli fout

De OTA-updatestap in `deploy.yml` sloeg stilzwijgend over wanneer `EXPO_TOKEN` ontbrak, zonder enige melding naar René — monteurs kregen dan geen app-update terwijl de deploy groen eindigde. Opgelost: de stap krijgt `continue-on-error: true` zodat een eas-cli-fout de deploy niet langer blokkeert, en een nieuwe stap "OTA-skip of -faalmelding e-mailen naar René" stuurt via dezelfde Microsoft 365/Graph-koppeling een gerichte e-mail wanneer (a) het token ontbreekt of (b) eas-cli faalt. De e-mail vermeldt de reden en de handmatige herstelstap. `PRODUCTION_RUNBOOK.md` gedocumenteerd onder "OTA-update bewaking".
