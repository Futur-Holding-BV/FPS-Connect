...
- De database wordt opgebouwd via het bestaande SCHEMA_01-nulpunt, de bestaande profiel- en testaccountseed en de genummerde migratierunner. De aanvraag-uploadrace en overige tests krijgen alleen in de bestaande teststap hetzelfde geïsoleerde database-adres via `AANVRAAG_TEST_DATABASE_URL`.
- Het acceptatieregister kiest bij gelijk gedateerde opdrachtdocumenten het hoogste numerieke revisienummer. Daardoor is voor UREN_01 revisie 3 met alle 18 acceptatiepunten leidend in plaats van de oude basisversie met 10 punten.
- De acceptatieregister-CI genereert het statusrapport vóór de V6-inhoudscontrole en ververst het na de volledige registerverificatie opnieuw uit de definitieve stand.
...
- Tabwissels blijven in de URL maar vervangen de actuele geschiedenisregel. De gewone browserknoppen Vorige en Volgende blijven daardoor uitsluitend de echte paginageschiedenis doorlopen.
- Appnavigatie via de nieuwe centrale bewaking beschermt onopgeslagen invoer. De gebouwnotities gebruiken deze poort aantoonbaar: annuleren behoudt tekst en route, opslaan-en-verlaten wacht op de opslag en bewust verlaten zonder opslag gaat naar het vaste bovenliggende scherm. Eén browserhistoriecoördinator bewaakt daarbij zowel het hoofdscherm als afzonderlijke geheugenbanen.
- De onboardingtopbalk heeft één centrale actie **Onboarding stoppen**. Na bevestiging verdwijnen het onafgeronde medewerkerconcept, bekende conceptgegevens, sessies en het speciaal daarvoor aangemaakte gebruikersaccount; zonder aangemaakt account wordt alleen de lokale invoer gewist.
- Hard verwijderen vereist nu een persistente server-side onboardingherkomst. Nieuwe account- en conceptaanmaak zetten die herkomst transactioneel, succesvolle medewerker- of externe-adviseurafronding wist haar in dezelfde transactie en gewone, actieve of afgeronde accounts worden met `409` geweigerd.
- Vóór verwijderen worden account en medewerker vergrendeld en worden alle actuele databaseverwijzingen gecontroleerd. Alleen expliciet bekende account-/onboardingartefacten mogen meegaan; een uren-, planning-, gebouw- of andere operationele koppeling blokkeert vóór enige cascade met een zichtbare `409`.
- De genummerde migratie, schema-driftcontrole, API-build, 123 gerichte regressietests en browserproeven zijn groen. De browserproef bewees account-zonder-concept, account-met-concept, sessie-intrekking, provenanceblokkade, operationele dependencyblokkade, herstel na verwijderen van die dependency en volledige opruiming van tijdelijke testdata.
...
- **Herstelbewijs zonder productierisico**: de herstelproef weigert sets buiten de dagstaffel, checksumfouten, symlinks en speciale bestanden; hij krijgt alleen minimale secretloze proefconfiguratie, gebruikt unieke tijdelijke containers/netwerken en laat health, login/2FA, documentopening en checksum hard falen. Achttien geïsoleerde regressieproeven dekken onder meer exit 23, SIGTERM/143, setbehoud, herinneringsdeduplicatie/retry, herstelpreflight, bestaande en interne symlinks, stale status, planning en hosttrust; productieclaims worden pas na de Actions-run in het meetrapport ingevuld.
- **VPS-hostidentiteit typevast gepind**: deploy en herstelworkflow accepteren uitsluitend één vooraf door een beheerder geverifieerde `SHA256`-fingerprint waarvan de actuele scanregel exact `ssh-ed25519` is. Een afwijkende fingerprint én correct gepinde RSA-/ECDSA-hostkeys stoppen vóór SSH en schrijven geen `known_hosts`.
- **Losstaande CI-laadfouten opgeheven zonder productieroutewijziging**: het herschikbewijs hergebruikt voortaan één vast testaccount en ruimt alleen zijn calculatiegegevens op, zodat append-only bevoegdheidsaudit niet meer door accountverwijdering wordt geraakt. De factuurverzendtest gebruikt `vi.hoisted()` voor alle mocks die in gehesen fabrieken nodig zijn en laadt in isolatie niet de externe werk-inbox; `facturen.ts` bleef ongewijzigd. De wegwerp-Postgres krijgt vóór de migratierunner het vaste SCHEMA_01-nulpunt plus de echte profielpresets, de statische hergraderingscontrole opent de database pas op het echte schrijfpad en de opleverproef leest na hergrading de actuele DB-stand in plaats van een oude API-momentopname.
...
- **Hersteld document doorloopt echte matrix- en contextautorisatie**: het tijdelijke hoofdbeheerderaccount krijgt in de geïsoleerde database exact één bestaand `Directie`-profiel en het gekozen document exact één tijdelijke koppeling aan een hersteld gebouw. De download bewijst daardoor opslagherstel via de normale document-, gebouw- en bevoegdhedenpoorten.
...
## 2026-08-20 — BANK_01: CAMT.053-bankafschriften automatisch afletteren

- **Eén veilige importmotor**: Administratie en Externe boekhouder kunnen CAMT.053 uploaden; een expliciet Microsoft-bankpostvak gebruikt exact dezelfde motor. ING, Rabobank en ABN AMRO zijn gedekt; MT940 staat apart als legacy-terugval.
- **Werkmaatschappij, eurovaluta en volledigheid fail-closed**: ieder afschrift-IBAN moet globaal exact één bankrekeningregistratie hebben. Rekening-, balans- en transactievaluta moeten expliciet EUR zijn; bedragen en credit-/debetrichting worden strikt en volledig geparseerd zonder impliciete valutaconversie. Transactiedetails moeten afzonderlijk of als som exact aansluiten op het omhullende entrybedrag. Statement- en importtotalen gebruiken `BigInt`, zodat ook grote elkaar opheffende mutatiereeksen niet stil op centen afronden; alle opgeslagen waarden passen vooraf aantoonbaar in `numeric(14,2)`. Het originele bestand wordt per betrokken werkmaatschappij gearchiveerd; interne saldofouten en aansluitingsfouten blokkeren het hele bestand, terwijl reeks-hiaten zichtbaar blijven en duurzaam worden gemeld.
- **Deterministisch afletteren**: alleen één kandidaat met betrouwbare referentie en exact volledig bedrag boekt verkoop-, inkoop- of betaalbatchposten automatisch af. Alle deel-, verzamel-, ontbrekende of meervoudige matches blijven met redenen op de werklijst; G-rekeningmutaties zijn herkenbaar en filterbaar.
- **Betaalbatchbewijs blijft onderscheidend**: alleen een volledig op het afschrift bewezen batch wordt **uitgevoerd**. De oudere handmatige stroom blijft zonder afschrift herkenbaar **bevestigd**; als een later afschrift alle regels exact bewijst, promoveert de batch pas dan naar **uitgevoerd**.
- **Crash- en duplicateveilig**: hashes, rekening+bankreferentie en mailbox/message/attachment zijn duurzaam uniek. Mailboxclaims hebben leases; een AccountView-POST stopt lokaal uiterlijk na 60 seconden en een claim kan pas na minimaal 15 minuten met exact hetzelfde claimtoken **onzeker** worden. Daarna kan hij alleen na expliciete externe controle worden bevestigd of vrijgegeven—nooit automatisch opnieuw verzonden.
- **Bestaande ketens hergebruikt**: Graph-token/ACL/stilstandbewaking, duurzame faalmail en de gedeelde AccountView-client/configuratie/logs blijven de enige integratiepaden. Autorisatie loopt uitsluitend via de nieuwe matrixmodule `bankafschriften`, toegekend aan de bestaande presets Administratie en Externe boekhouder.
- **Mailboximport blijft van OAuth-configuratie ontkoppeld**: het laden van de Graph-mailboxhelpers vraagt niet langer direct om `SESSION_SECRET`. Alleen het daadwerkelijk ondertekenen of verifiëren van een OAuth-state leest de secret en faalt bij ontbrekende of te korte configuratie nog steeds expliciet en gesloten.
- **Bewijs**: 168 gerichte tests groen, inclusief de vaste verhouding tussen transporttimeout en claimverval en adversarial vreemde-valuta-, kapotte-decimaal-, richting-, entry/detailaansluitings-, DB-decimaal- en BigInt-aggregatietests; echte development-DB-ketenproef groen voor multi-account/multi-dag, duplicate, onbekend/dubbelzinnig IBAN, niet-EUR, afwijkend transactiedetail en vals afgerond aggregaat zonder opslag/afboeking, saldo/reeks/backfill, exacte, ambigue en handmatige verkoop-/batchmatching, batchsluiting (ook later bankbewijs na handmatige bevestiging), G-rekening, mailboxlease/Graph-fout en AccountView-crashherstel; API- en webtypechecks, productiebuilds en schema-driftcontrole groen; browserproef groen voor bevoegd/403, uploadlabels, filters en de volledige `onzeker`-herstelinteractie; onafhankelijke architectuurreview PASS.
## 2026-08-22 — REGISTER_01: reproduceerbare productiesnapshot

- De bestaande registerexport schrijft een uitsluitend gelezen, stabiel gesorteerde JSON-momentopname naar `docs/metingen/ACCEPTATIEREGISTER_PRODUCTIE.json`. De export bevat de database-exporttijd, de werkboomcommit, de vier standentotalen, de unieke-sleutelcontrole en alle registervelden; productie-uitvoer vereist bewust `--productie` plus `PROD_LEZEN_TOEGESTAAN=1`.
...
## 2026-08-20 — LOON_02A fiscaal loonfundament

- **Afgeschermde grondslag naast de loonstroom**: een nieuwe Loonfundament-module beheert inhoudingsplichtigen, meerdere inkomstenverhoudingen per medewerker/werkgever, gedateerde loonafspraken in centen, officiële jaarparameters en maand-/vierwekenloonstaten. SCAB-mail, salarisarchief, SEPA, loonoutput en loonstrookpublicatie zijn niet gewijzigd.
- **Bindende CAO-catalogus**: werkgevers en aanstellingen verwijzen naar catalogusrecords. FPS Bouw en FPS Brandpreventie staan op Metaal & Techniek; FPS Bouw en Renovatie staat op Bouw & Infra. Onbekende of tegenstrijdige historische vrije tekst blijft zichtbaar als migratiebevinding.
- **Uitsluitend hoofdbeheerder en Externe boekhouder**: iedere API-route combineert het `loonfundament`-niveau met een niet-toekenbare identiteitspoort voor de hoofdbeheerder of het systeemprofiel Externe boekhouder. Een gebruiker met alleen handmatig `loonfundament:4` krijgt 403; menu en webroute gebruiken dezelfde servervlag en mounten geen datahooks voor onbevoegden.
- **Officiële 2026-set geladen**: de gepinde Belastingdienst-bronset bestaat uit de primaire XLSX, rekenvoorschriften versie 2, parameterbijlage, gegevensspecificaties, loonbelastingtabellen, cijferbijlage en Handboek versie maart. De primaire XLSX leverde 7.220 herleidbare parameterrecords; bestandsnamen en hashes staan in `docs/loonfundament.md`.
- **Geen fiscale constants in programmacode**: de import valideert HTTPS-host, redirect, type, volledige bronset en SHA-256, parseert naar exacte werkblad-/celvindplaatsen en promoveert alleen transactioneel onder een jaarlock. De nieuwste set is beslissend; ontbrekende, gewijzigde, onvolledige of niet-herleidbare data blokkeert gereedheid zonder jaarfallback.
- **LOON_02A rekent nadrukkelijk niet**: tijdvakregels kunnen alleen als `niet_berekend` met lege waarden en cumulatieven worden opgeslagen. Zowel API-contract als databaseconstraint blokkeren clientgeschreven rekenuitkomsten totdat LOON_02B een server-side rekenpad levert.
- **Database-invarianten in beide richtingen**: een inkomstenverhouding moet bij medewerker en werkgever van de aanstelling passen, en een gekoppelde aanstelling kan daarna niet stil naar een andere medewerker of werkgever worden verplaatst.
- **Onveranderlijke historie en geldige tijdvakken**: loonafspraken zijn databasebreed append-only. Maandregels moeten exact hun kalendermaand volgen (maximaal 12); vierwekenloonstaten ondersteunen maximaal 13 tijdvakken. Alle perioden blijven binnen hetzelfde kalenderjaar en de loonstaatgrondslag staat na de eerste regel vast.
- **Bewijs**: 67 echte PostgreSQL-controles bewijzen CAO-defaults, meervoudige inkomstenverhoudingen, gedateerde centenafspraken, de wederzijdse aanstellingspoort, manifestpinning, jaarblokkade, LOON_02A-rekenblokkade, tijdvakinvarianten en de profielidentiteit. De volledige regressiesuite telt 658 geslaagde tests en 2 bestaande skips.
- **CI-bewijs is pakketzuiver**: `@workspace/api-server` declareert nu zelf de `tsx`-runner die zijn verificatiescripts uitvoeren. Daardoor werkt de Loonfundament-proef ook na een schone frozen installatie en leunt hij niet op toevallig lokaal aanwezige tooling uit een ander workspacepakket.


## 2026-08-21 — Connect-assistent overal beschikbaar en strikt bevoegd

- Op desktop staat op ieder normaal Connect-scherm bovenin **Vraag iets over dit scherm**. De assistent opent naast de werkpagina, blijft gemount bij sluiten en houdt gesprek en actuele scherm-/objectcontext vast tijdens clientnavigatie. Op telefoon blijft `/assistent` de enige ervaring en verschijnt geen zwevend paneel.
- Gesprekken staan server-side en zijn uniek geïsoleerd op de echte actor, de effectief bekeken gebruiker, de effectieve rol én een cryptografische momentopname van module-, object- en gebouwrechten. Ook intrekking binnen dezelfde rol laadt daardoor onmiddellijk een leeg gesprek; eerder ruimere antwoorden komen niet terug via historie of vervolgvragen.
- Een gesloten register van uitsluitend leesgereedschap dekt projecten, gebouwen, calculaties, offertes, opdrachten, uren, voorraad, leveranciers en facturen. Iedere uitvoerder controleert de effectieve module-, object- en gebouwscope vóór de inhoudelijke query; onbekende of schrijvende gereedschappen bestaan niet.
- De contextmotor autoriseert iedere knoop vóór de inhoudelijke resolver of cache-read. Facturen volgen dezelfde modulebrede `financieel`-poort als de gewone factuurschermen; uren volgen dezelfde eigen-uren-/`personeel:1`-scheiding als de urenroutes.
- Feitelijke modeluitvoer is fail-closed: het model levert atomische claims met door de server uitgegeven bron-id’s, de server weigert ontbrekende of onbekende bronnen en toont per claim bronnummer(s) bij klik-veilige interne Connect-links. Een generieke bron wordt niet meer achteraf op een ongeciteerd antwoord geplakt.
- Vraag, antwoord, effectieve identiteit/rol, autorisatiemomentopname, scherm-/objectcontext, tooltoelatingen, bronselectie en weigeringen worden controleerbaar geaudit. Voor iedere feitelijke claim bewaart de audit bovendien de claim→bron-id-koppeling en uitsluitend de werkelijk gebruikte, begrensde broninhoud; een succesvolle wisseling en auditregel worden atomair opgeslagen. Auditfalen geeft geen antwoord vrij.
- Bestaande routes voor AI-invullen, AI-veldcorrecties en chat-leesbevestigingen controleren nu ook formuliermodule, object-/gebouwscope of werkelijk deelnemerschap en weigeren fail-closed.
- Regressiebewijs dekt broncontract, actor-/rolisolatie, intrekking van module-/object-/gebouwrechten, tijdgebonden rechten, begrensd auditbewijs, negen leesdomeinen, scopeweigering vóór resolvers, routehardening, desktoppersistentie, klikbare citaties en de mobiele volledige pagina.
