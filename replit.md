...
- **Governance & Approval Engine** — kernmotor + pilot inkoopbon (10 juli 2026) — generieke goedkeuringsmotor (beleidsregels + aanvragen als state machine, bewust niet gekoppeld aan één proces); eerste koppeling is de inkoopbon-status-transitie (blokkeert direct PATCHen met 422 zodra een beleidsregel geldt, motor voert de transitie zelf uit na goedkeuring); beheerscherm `/beheer/goedkeuringsbeleid`. Overige koppelingen (offertes, HRM) volgen later op dezelfde motor. Zie [docs/roadmap/gebouwd.md](docs/roadmap/gebouwd.md)
...
- **V1.5** — Rapportenmodule — **grotendeels gebouwd** (gepersisteerde definitieve rapporten + bevriezing via V1.4; centrale rapportenbibliotheek met statusfilter + vrije-tekst zoekfunctie op titel/gebouw/type/opsteller, 8 juli 2026; reactietermijn-statusmachine met vier zichtbare statussen — Definitief verzonden/Reactietermijn loopt/Termijn verstreken/Vervangen door nieuwe versie — incl. automatisch "vervangen" zetten bij een nieuwe versie, 8 juli 2026); restscope: koppeling naar CRM/onderhoud/klantportaal
