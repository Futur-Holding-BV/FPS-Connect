...
- **V1.4** — Opleverrapportage (voorblad, rapportopmaak, spotselectie, e-mailselectie, bijlagenpakket, definitief maken) — **in aanbouw** met formeel akkoord; bouwt voort op de bestaande live-rapportage in `print.tsx` (niet opnieuw vanaf nul)
...
- **Document Design System** — modulebrede documentmotor: drie templatefamilies (A klantdocumenten, B HRM/juridisch, C interne operationele), per-werkmaatschappij centraal beheer (bouwt op de Werkgever-entiteit), versiebeheer + PDF + latere digitale ondertekening. Eerste oplevering: visuele basis + 5 voorbeeldtemplates met dummy-content. Vastgelegd; pas bouwen ná formeel akkoord. Detail in [docs/roadmap/document-design-system.md](docs/roadmap/document-design-system.md)
