...
// opzet/schoonmaak. Draaien: pnpm --filter @workspace/scripts run verificatie-externe-adviseur
import { weigerProductieVoorSchrijvendScript } from "./lib/prodGuard";
import { authenticator } from "otplib";
import { and, desc, eq, like } from "drizzle-orm";
import bcrypt from "bcryptjs";
import {
  db, gebruikersTable, externeAdviseursTable, medewerkersTable, werkbakItemsTable,
} from "@workspace/db";
import {
...
// Dit script maakt, wijzigt en verwijdert fixturedata. De vrijstelling voor
// puur lezende productiescripts mag dit pad daarom nooit openzetten.
...
async function schoonOp() {
  // Werkbak-items verwijzen bewust niet met een foreign key naar de bron. Ruim
  // daarom ook een eventueel item van een afgebroken vorige run op, vóór het
  // fixture-account verdwijnt.
  await db.delete(werkbakItemsTable)
    .where(like(werkbakItemsTable.titel, "%Bewijs Externe Adviseur%"));
...
async function main() {
  ...
  try {
    ...
    // Bewijs 9-12: de werkbakvoeder bewaakt precies de 14-dagengrens, ook na
    // de vervaldatum, en reconcilieert zodra verlenging of deactivering de bron
    // uit de actuele set haalt.
    const datumOver = (dagen: number) => new Date(Date.now() + dagen * 86400000).toISOString().slice(0, 10);
    const laadWerkbakItem = () => db.select().from(werkbakItemsTable)
      .where(and(
        eq(werkbakItemsTable.dedupSleutel, `externe_adviseur:${adviseurId}:toegang`),
        eq(werkbakItemsTable.bron, "verloopdatum"),
      ))
      .orderBy(desc(werkbakItemsTable.id))
      .limit(1);

    const binnenVeertien = await api(admin, "PATCH", `/externe-adviseurs/${adviseurId}`, { toegang_tot: datumOver(14) });
    check("toegang precies binnen 14 dagen gezet (200)", binnenVeertien.status === 200, JSON.stringify(binnenVeertien.json));
    const draaiBinnenVeertien = await api(admin, "POST", "/werkbak/bewaking/draai");
    const [itemBinnenVeertien] = await laadWerkbakItem();
    check("14-dagengrens → open werkbakitem", draaiBinnenVeertien.status === 200 && itemBinnenVeertien?.status === "open",
      `${draaiBinnenVeertien.status} ${JSON.stringify(itemBinnenVeertien)}`);

    const verlopen = await api(admin, "PATCH", `/externe-adviseurs/${adviseurId}`, { toegang_tot: gisteren });
    check("verlopen toegangsdatum gezet (200)", verlopen.status === 200, JSON.stringify(verlopen.json));
    const draaiVerlopen = await api(admin, "POST", "/werkbak/bewaking/draai");
    const [itemVerlopen] = await laadWerkbakItem();
    check("verlopen toegang → werkbakitem blijft open", draaiVerlopen.status === 200
      && itemVerlopen?.status === "open" && itemVerlopen.titel.includes("verlopen"), JSON.stringify(itemVerlopen));

    const verlengd = await api(admin, "PATCH", `/externe-adviseurs/${adviseurId}`, { toegang_tot: datumOver(15) });
    check("toegang buiten bewakingsvenster verlengd (200)", verlengd.status === 200, JSON.stringify(verlengd.json));
    const draaiNaVerlenging = await api(admin, "POST", "/werkbak/bewaking/draai");
    const [itemNaVerlenging] = await laadWerkbakItem();
    check("verlenging buiten venster → werkbakitem automatisch afgehandeld",
      draaiNaVerlenging.status === 200 && itemNaVerlenging?.status === "afgehandeld",
      `${draaiNaVerlenging.status} ${JSON.stringify(itemNaVerlenging)}`);

    const opnieuwVerlopen = await api(admin, "PATCH", `/externe-adviseurs/${adviseurId}`, { toegang_tot: gisteren });
    const draaiVoorDeactivering = await api(admin, "POST", "/werkbak/bewaking/draai");
    const [itemVoorDeactivering] = await laadWerkbakItem();
    check("verlopen toegang vóór deactivering → nieuw open werkbakitem",
      opnieuwVerlopen.status === 200 && draaiVoorDeactivering.status === 200 && itemVoorDeactivering?.status === "open",
      `${opnieuwVerlopen.status}/${draaiVoorDeactivering.status} ${JSON.stringify(itemVoorDeactivering)}`);
    const gedeactiveerd = await db.update(gebruikersTable)
      .set({ actief: false })
      .where(eq(gebruikersTable.id, gebruikerId))
      .returning({ id: gebruikersTable.id });
    const draaiNaDeactivering = await api(admin, "POST", "/werkbak/bewaking/draai");
    const [itemNaDeactivering] = await laadWerkbakItem();
    check("account deactiveren → open werkbakitem automatisch afgehandeld",
      gedeactiveerd.length === 1 && draaiNaDeactivering.status === 200 && itemNaDeactivering?.status === "afgehandeld",
      `${gedeactiveerd.length}/${draaiNaDeactivering.status} ${JSON.stringify(itemNaDeactivering)}`);
  } finally {
    await schoonOp();
  }
