...
// opdrachtbestand in docs/opdrachten — het vulscript kan 'm dus niet vinden).
import { db, acceptatieRegisterTable } from "@workspace/db";
import { sql } from "drizzle-orm";

const punten: { nr: number; oms: string; stand: string; bewijs: string }[] = [
  { nr: 1, oms: "Register bestaat met vier standen (gehaald, niet_gebouwd, onbewezen, wacht_op_rene) en is gevuld uit de Acceptatie-paragrafen van alle opdrachten in docs/opdrachten", stand: "gehaald", bewijs: "migratie 0093 + scripts/src/vul-acceptatieregister.ts (52 opdrachten, 424 punten)" },
