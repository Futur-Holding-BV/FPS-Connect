...
import 
{
 eq, desc, and, inArray, ne, isNotNull 
}
 from "drizzle-orm"
;
...
import 
{
 stuurRapportBeschikbaarMelding 
}
 from "../services/email"
;
...
import 
{
 bouwNieuweVersieWaarden, dedupeerPartijEmails 
}
 from "../lib/rapport-helpers"
;
...
;
    ...
    res.json(rijen.map(r => mapRapport(r.r, 
{
 aangemaaktDoorNaam: r.naam, gebouwNaam: r.gebouwNaam, werkbonNummer: r.werkbonNummer 
}
)))
;
...
;
    ...
    const rijen = await db
      .select(
{
 r: opleverrapportenTable, naam: gebruikersTable.naam 
}
)
      .from(opleverrapportenTable)
      .leftJoin(gebruikersTable, eq(opleverrapportenTable.aangemaaktDoor, gebruikersTable.id))
...
;
    ...
    const 
{
 rapport_type, titel, werkbon_id, secties, spot_selectie, bijlagen_ids, tekening_ids, reactietermijn_datum 
}
 =
      req.body as 
{
...
;
    ...
    res.json(mapRapport(rij.r, 
{
 aangemaaktDoorNaam: rij.naam 
}
))
;
    ...
    if (huidig.status !== "concept") 
{
      ...
      res.status(409).json(
{
 error: "Alleen concept-rapporten kunnen worden bewerkt" 
}
)
;
...
;
    ...
    const 
{
 titel, werkbon_id, secties, spot_selectie, bijlagen_ids, tekening_ids, reactietermijn_datum 
}
 =
      req.body as 
{
 ...
 : 
{
        ...
        ...(werkbon_id !== undefined ? 
{
 werkbonId: werkbon_id 
}
 : 
{
}
    ...
    if (huidig.status !== "concept") 
{
      ...
      res.status(409).json(
{
 error: "Definitieve rapporten kunnen niet worden verwijderd" 
}
)
;
    ...
    if (huidig.status !== "concept") 
{
      ...
      res.status(409).json(
{
 error: "Rapport is al definitief of gearchiveerd" 
}
)
;
    ...
    if (isNaN(dagen) || dagen < 1 || dagen > 365) 
{
      ...
      res.status(400).json(
{
 error: "reactietermijn_dagen moet tussen 1 en 365 liggen" 
}
)
;
...
;
    ...
    let bevrorenRevisies: Record<string, 
{
 revisie_nummer: number | null
;
 naam: string 
}
> = 
      ...
      try 
{
        ...
        const docs = await db
          .select(
{
 id: documentenTable.id, revisieNummer: documentenTable.revisieNummer, naam: documentenTable.naam 
}
)
        ...
        for (const d of docs) 
{
          ...
          bevrorenRevisies[String(d.id)] = 
{
 revisie_nummer: d.revisieNummer ?? null, naam: d.naam 
}
;
 ...
 catch 
{
        ...
        // Bevriezing-details zijn best-effort
;
 definitief maken gaat door
...
;
        ...
        const partijen = dedupeerPartijEmails(
          await db
            .select(
{
 naam: gebouwPartijenTable.naam, email: gebouwPartijenTable.email 
}
)
...
, "Klant-notificaties rapport-beschikbaar gefaald (niet-kritiek)")
;
    ...
    if (huidig.status !== "definitief") 
{
      ...
      res.status(409).json(
{
 error: "Alleen definitieve rapporten kunnen worden vervangen door een nieuwe versie" 
}
)
;
    ...
    if (huidig.vervangenDoorId != null) 
{
      ...
      res.status(409).json(
{
 error: "Rapport is al vervangen door een nieuwe versie" 
}
)
;
...
;
    ...
    await db
      ...
      .set(
{
 vervangenDoorId: nieuw.id, bijgewerktOp: nu 
}
)
      .where(eq(opleverrapportenTable.id, rapportId))
;
    ...
    if (huidig.certificaatGeaccordeerd) 
{
      ...
      res.status(409).json(
{
 error: "Certificaat is al geaccordeerd" 
}
)
;
      ...
      .set(
{
        ...
        ...(maanden && !isNaN(maanden) && maanden > 0 ? 
{
 certificaatGarantieMaanden: maanden 
}
 : 
{
}
...
// KLANTLOOS_01: de klant logt niet meer in op Connect
;
 deze route registreert
    ...
    if (huidig.status !== "definitief") 
{
      ...
      res.status(409).json(
{
 error: "Alleen definitieve rapporten kunnen worden bevestigd" 
}
)
;
    ...
    if (huidig.klantReactieOp != null) 
{
      ...
      res.status(409).json(
{
 error: "Er is al een reactie geregistreerd voor dit rapport" 
}
)
;
    ...
    if (!reactie_type || reactie_type !== "ontvangst_bevestigd") 
{
      ...
      res.status(400).json(
{
 error: "reactie_type moet 'ontvangst_bevestigd' zijn" 
}
)
;
...
;
      ...
      try 
{
 breedte = font.widthOfTextAtSize(kandidaat, size)
;
 
}
 catch 
{
 breedte = kandidaat.length * size * 0.5
;
 
}

      if (breedte > maxWidth && huidig) 
{
 regels.push(huidig)
;
 huidig = woord
...
;
    ...
    if (!rapport) return void res.status(404).json(
{
 error: "Rapport niet gevonden" 
}
)
...
{
 error: "Geen bijlagen geselecteerd in dit rapport" 
}
)
;
...
;
        ...
        const bronDoc   = await PDFDocument.load(buffer, 
{
        ...
        if (aantalPag <= 5) 
{
          ...
          const kopieën = await outputDoc.copyPages(bronDoc, Array.from(
{
 length: aantalPag 
}
, (_, i) => i))
...
, "AI-samenvatting bijlage mislukt")
;
...
{
 x: 0, y: A4H - 80, width: A4B, height: 80, color: rgb(0.945, 0.231, 0.051) 
}
)
;
...
{
 x: 50, y, font: fontReg, size: 10, color: rgb(0.1, 0.1, 0.1) 
}
)
;
 ...
 catch (docErr) 
{
        ...
        req.log.warn(
{
 docId: doc.id, err: docErr 
}
, "Bijlage overgeslagen bij bundle-generering")
;
...
{
 error: "Geen van de bijlagen kon worden verwerkt" 
}
)
;
...
;
    ...
    res.status(500).json(
{
 error: "Serverfout bij genereren bijlagenbundel" 
}
)
;
