...
const TYPE_META: Record<string, { label: string; kleur: string; icoon: React.ElementType }> = {
  ...
  uitvoeringsrisico:      { label: "Uitvoeringsrisico",     kleur: "text-orange-700 bg-orange-50 border-orange-200", icoon: ShieldAlert },
  inkoopactie_nodig:      { label: "Inkoopactie nodig",     kleur: "text-blue-700 bg-blue-50 border-blue-200",     icoon: ShoppingCart },
  planningrisico:         { label: "Planningrisico",        kleur: "text-purple-700 bg-purple-50 border-purple-200", icoon: CalendarClock },
  kostenrisico:           { label: "Kostenrisico",          kleur: "text-amber-700 bg-amber-50 border-amber-200",   icoon: TrendingUp },
  ontbrekende_voorbereiding: { label: "Ontbrekende voorbereiding", kleur: "text-slate-700 bg-slate-50 border-slate-200", icoon: ClipboardList },
  besparingskans:         { label: "Besparingskans",        kleur: "text-green-700 bg-green-50 border-green-200",   icoon: TrendingDown },
};
...
function AdviesKaart({
  ...
  const meta = TYPE_META[advies.type] ?? TYPE_META["waarschuwing"]!;
...
export default function AiSeniorWerkvoorbereiderPanel({ opdrachtId }: { opdrachtId: number }) {
  const qc = useQueryClient();
  const [filter, setFilter] = useState<FilterOptie>("actief");

  const { data: adviezen = [], isLoading } = useListWbAdviezen(opdrachtId, {
    query: { queryKey: getListWbAdviezenQueryKey(opdrachtId) },
  });

  const analyseMut = useAiSeniorAnalyseWerkbegroting({
    mutation: {
      onSuccess: () => {
        qc.invalidateQueries({ queryKey: getListWbAdviezenQueryKey(opdrachtId) });
      },
    },
  });

  const updateMut = useUpdateWbAdvies({
    mutation: {
      onSuccess: () => {
        qc.invalidateQueries({ queryKey: getListWbAdviezenQueryKey(opdrachtId) });
      },
  ...
  function handleUpdate(adviesId: number, update: { status?: string; notitie?: string | null }) {
    updateMut.mutate({ id: opdrachtId, adviesId, data: update });
  }
  ...
  return (
    <div className="border rounded-lg flex flex-col overflow-hidden" style={{ maxHeight: 600 }}>
      {/* Header */}
      <div className="p-3 border-b shrink-0 bg-background">
        <div className="flex items-center gap-2 mb-2">
          <HardHat className="h-4 w-4 text-orange-600 shrink-0" />
          <span className="text-sm font-semibold">AI Senior Werkvoorbereider</span>
          {aantalHoog > 0 && (
        ...
        <Button
          className="w-full"
          size="sm"
          onClick={() => analyseMut.mutate({ id: opdrachtId })}
        ...
        >
          ...
          {analyseMut.isPending ? "Analyseren..." : adviezen.length === 0 ? "Analyseer werkbegroting" : "Heranalyseer"}
        </Button>
        {analyseMut.isPending && (
          <p className="text-xs text-muted-foreground mt-1.5 text-center">
            Senior werkvoorbereider kijkt mee...
          </p>
        )}
      </div>
      ...
      {/* Filter tabs */}
      {adviezen.length > 0 && (
        <div className="flex gap-0 border-b shrink-0 bg-background">
        ...
        <div className="p-3 space-y-2">
          ...
          {!isLoading && adviezen.length === 0 && !analyseMut.isPending && (
            <div className="text-center py-6 space-y-2">
              <HardHat className="h-8 w-8 mx-auto text-muted-foreground/40" />
              <p className="text-xs text-muted-foreground">
                Klik op "Analyseer werkbegroting" om de AI Senior Werkvoorbereider te laten meekijken.
              </p>
              <p className="text-xs text-muted-foreground/60">
                De AI controleert uitvoerbaarheid, inkoop, planning en ontbrekende voorbereidingen.
              </p>
            </div>
          )}
