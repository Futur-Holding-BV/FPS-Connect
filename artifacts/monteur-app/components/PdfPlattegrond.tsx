...
(function(){
  ...
  function renderMarkers(){
    ...
    spots.forEach(function(s){
      ...
      el.appendChild(lab);
      (function(id){
        el.addEventListener('click', function(ev){ ev.stopPropagation(); post({ type:'spot', id:id }); });
      })(s.id);
      wrap.appendChild(el);
    });
  }
  ...
  function loadPdf(){
    ...
    fetch(CFG.domein + '/api/storage' + CFG.url, { headers: { Authorization: 'Bearer ' + CFG.token } })
      ...
      .then(function(page){
        ...
        pageW = Math.ceil(vp.width);
        pageH = Math.ceil(vp.height);
  ...
  var startD = 0, startScale = 1, midX = 0, midY = 0, midIx = 0, midIy = 0;
  ...
  stage.addEventListener('touchmove', function(e){
    ...
    if (e.touches.length === 1 && panning) {
      ...
      scale = ns;
      tx = midX - midIx * scale; ty = midY - midIy * scale; apply();
    }
  }, { passive:false });
  ...
  window.__setPlace = function(v){ placeMode = !!v; if (v) wrap.classList.add('placing'); else wrap.classList.remove('placing'); };
