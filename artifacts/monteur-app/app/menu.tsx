...
export default function MenuScherm() {
  ...
  const categoriekleur = opdracht
    ? (CATEGORIE_KLEUREN[opdracht.toolbox_categorie ?? ""] ?? CATEGORIE_KLEUREN.anders)
    : CATEGORIE_KLEUREN.anders;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.dark }}>
      ...
      >
        ...
        >
          <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined}>
            ...
            >
              {/* Categorie streep */}
              <View style={{ height: 5, backgroundColor: categoriekleur, borderTopLeftRadius: c.radius + ruimte.xs, borderTopRightRadius: c.radius + ruimte.xs }} />

              <ScrollView
                style={{ paddingHorizontal: ruimte.l + ruimte.xs }}
                contentContainerStyle={{ paddingTop: ruimte.l + 2, paddingBottom: ruimte.s }}
                showsVerticalScrollIndicator={false}
              >
                {/* Status badge */}
                <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: ruimte.m }}>
                  <View
                    style={{
                      backgroundColor: opdracht?.is_verplicht ? c.destructive + "1A" : c.warning + "1A",
                      borderRadius: c.radius / 2,
                      paddingHorizontal: ruimte.s + 2,
                      paddingVertical: ruimte.xs,
                    }}
                  >
                    <Text style={{ fontSize: 12, fontFamily: "Inter_600SemiBold", color: opdracht?.is_verplicht ? c.destructive : c.warning }}>
                  ...
                  {opdracht?.kan_uitstellen && (
                    ...
                    >
                      <Text style={{ fontSize: 12, fontFamily: "Inter_400Regular", color: c.mutedForeground, textDecorationLine: "underline" }}>
                        Uitstellen
                      </Text>
                    </Pressable>
                  )}
                ...
                {/* Titel */}
                <Text style={{ fontSize: 20, fontFamily: "Inter_700Bold", color: c.foreground, marginBottom: ruimte.xs }}>
                  {maandLaden ? "Laden..." : (opdracht?.toolbox_titel ?? "")}
                </Text>

                {/* Maand label */}
                {opdracht && (
                  <Text style={{ fontSize: 13, color: c.mutedForeground, fontFamily: "Inter_400Regular", marginBottom: ruimte.m + 2 }}>
                ...
                {opdracht?.toolbox_intro ? (
                  <Text style={{ fontSize: 14, color: c.foreground, fontFamily: "Inter_400Regular", lineHeight: 21, marginBottom: ruimte.l + ruimte.xs }}>
                    {opdracht.toolbox_intro}
                  </Text>
                ) : null}
                ...
                >
                  <Ionicons name="book-outline" size={15} color={c.foreground} />
                ...
                {/* Vraag veld */}
                <Text style={{ fontSize: 13, fontFamily: "Inter_600SemiBold", color: c.foreground, marginBottom: ruimte.xs + 2 }}>
              ...
              {/* Actieknoppen */}
              <View style={{ paddingHorizontal: ruimte.l + ruimte.xs, paddingTop: ruimte.s }}>
                <Pressable
                  onPress={voltooien}
                  disabled={isBezig}
                  style={({ pressed }) => ({
                    backgroundColor: isBezig ? c.muted : c.primary,
                    opacity: !isBezig && pressed ? 0.85 : 1,
                    borderRadius: c.radius,
      ...
      >
        <View style={{ flexDirection: "row", alignItems: "center", gap: ruimte.m, flexShrink: 1 }}>
          <View
            style={{
        ...
        <View style={{ flexDirection: "row", gap: ruimte.xs + 2, alignItems: "center" }}>
          <Pressable
      ...
      {jarigeCollegas.length > 0 && (
        <View
          style={{
            marginHorizontal: ruimte.l + ruimte.xs,
        ...
        >
          <Text style={{ fontSize: 12, fontFamily: "Inter_600SemiBold", color: c.darkMuted, marginBottom: ruimte.s }}>
            🎂 Vandaag jarig
          </Text>
          <View style={{ flexDirection: "row", flexWrap: "wrap", gap: ruimte.s + 2 }}>
            {jarigeCollegas.map((m) => (
              <View key={m.medewerker_id} style={{ alignItems: "center", width: 64 }}>
                ...
                <Text
                  numberOfLines={1}
                  style={{ color: c.darkForeground, fontSize: 11, fontFamily: "Inter_400Regular", marginTop: ruimte.xs, textAlign: "center" }}
                >
                  {m.naam.split(" ")[0]}
                </Text>
              </View>
...
// Bewuste categoriekleuren: de dunne kleurstreep bovenaan de maandopdracht-modal
// codeert de toolbox-categorie. Dit zijn géén gewone UI-kleuren maar vaste
// betekeniskleuren per categorie (analoog aan de web-toolboxen); het palet kent
// hiervoor geen tokens, daarom staan ze hier expliciet en gedocumenteerd.
