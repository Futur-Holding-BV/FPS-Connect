    ...
    try {
      if (actie === "uitgifte") {
        await uitgifte.mutateAsync({
          data: {
            regels: [{ artikel_id: artikelId, hoeveelheid: aantal }],
          },
        });
        Alert.alert(
          "Uitgifte geregistreerd",
          `${aantal} ${eenheidLabel(artikel?.eenheid)} van "${artikel?.naam}" uitgeleverd.`,
          [{ text: "OK", onPress: onSluit }],
        );
      } else {
        await retour.mutateAsync({
          data: {
            regels: [{ artikel_id: artikelId, hoeveelheid: aantal, conditie: "goed" }],
          },
        });
        Alert.alert(
          "Retour geregistreerd",
          `${aantal} ${eenheidLabel(artikel?.eenheid)} van "${artikel?.naam}" ingenomen.`,
          [{ text: "OK", onPress: onSluit }],
        );
      }
    } catch (err: unknown) {
      const msg =
        err instanceof Error ? err.message : "Controleer je verbinding en probeer het opnieuw.";
    ...
    >
      ...
      >
            ...
            >
              ...
              <Text
                style={{
                  ...
                  color: vrij !== null && vrij <= (artikel.minimum_voorraad ?? 0) ? "#dc2626" : "#16a34a",
                }}
              >
                {vrij !== null ? `${vrij} ${eenheidLabel(artikel.eenheid)}` : "—"}
              </Text>
            </View>
            {artikel.minimum_voorraad !== null && artikel.minimum_voorraad !== undefined ? (
              <View
                style={{
                  flex: 1,
                  minWidth: 100,
        ...
        <View style={{ flexDirection: "row", gap: 10, marginBottom: 20 }}>
          <Pressable
            onPress={() => setActie("uitgifte")}
            style={({ pressed }) => ({
              flex: 1,
              paddingVertical: 12,
              borderRadius: 10,
              alignItems: "center",
              backgroundColor: actie === "uitgifte" ? c.primary : (pressed ? "#f3f4f6" : c.card),
              borderWidth: 1.5,
              borderColor: actie === "uitgifte" ? c.primary : c.border,
            })}
          >
            <Ionicons
              name="arrow-up-circle-outline"
              size={20}
              color={actie === "uitgifte" ? "#fff" : c.mutedForeground}
            />
            <Text
              style={{
                marginTop: 4,
                ...
                color: actie === "uitgifte" ? "#fff" : c.text,
              }}
            >
              Uitgifte
            </Text>
          ...
          <Pressable
            onPress={() => setActie("retour")}
            style={({ pressed }) => ({
              flex: 1,
              paddingVertical: 12,
              borderRadius: 10,
              alignItems: "center",
              backgroundColor: actie === "retour" ? c.primary : (pressed ? "#f3f4f6" : c.card),
              borderWidth: 1.5,
              borderColor: actie === "retour" ? c.primary : c.border,
            })}
          >
            <Ionicons
              name="arrow-down-circle-outline"
              size={20}
              color={actie === "retour" ? "#fff" : c.mutedForeground}
            />
            <Text
              style={{
                marginTop: 4,
        ...
        >
          {bezig ? (
            ...
            <>
              <Ionicons
                name={actie === "uitgifte" ? "arrow-up-circle" : "arrow-down-circle"}
                size={20}
                color="#fff"
              />
              <Text style={{ fontSize: 16, fontFamily: "Inter_700Bold", color: "#fff" }}>
                {actie === "uitgifte" ? "Uitgifte bevestigen" : "Retour bevestigen"}
              </Text>
            </>
          )}
        </Pressable>
...
export default function MagazijnScanScherm() {
  ...
  const [modus, setModus] = useState<Modus>("scan");
  const [gevondenArtikelId, setGevondenArtikelId] = useState<number | null>(null);
            ...
            >
              {scanBezig ? (
                ...
                >
                  ...
                  <Text style={{ color: "#fff", fontSize: 13, fontFamily: "Inter_600SemiBold", flexShrink: 1 }}>
                    {scanFout}
                  </Text>
                </View>
              ) : (
