...
const stappen = [
  ...
  {
    beschrijving: "gebruiker_profielen koppeltabel aanmaken (P2 meerdere rollen per gebruiker, incl. UNIQUE-paar en indexen)",
    sql: `
      ...
      BEGIN
        IF NOT EXISTS (
          ...
          WHERE conname = 'gebruiker_profielen_gebruiker_id_profiel_id_unique'
        ) THEN
          ALTER TABLE gebruiker_profielen
            ADD CONSTRAINT gebruiker_profielen_gebruiker_id_profiel_id_unique
            UNIQUE (gebruiker_id, profiel_id);
        END IF;
      END $$;
