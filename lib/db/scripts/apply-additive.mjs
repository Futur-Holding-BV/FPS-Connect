...
// Constraint-stappen die FATAAL falen (process.exit(1)) als ze mislukken.
// Vóór elke constraint wordt expliciet op dubbele rijen gecheckt; bij dubbele
// rijen wordt duidelijk gemeld welke combinaties het betreft en stopt het script
// zodat de deployment niet met een kapot schema doorgaat.
const constraintStappen = [
  {
    naam: "gebruiker_profielen_gebruiker_id_profiel_id_unique",
    tabel: "gebruiker_profielen",
    kolommen: ["gebruiker_id", "profiel_id"],
    beschrijving: "UNIQUE (gebruiker_id, profiel_id) op gebruiker_profielen",
  },
];
...
async function main() {
  ...
  // ── Constraint-stappen (fataal bij fout of bij aanwezige dubbele rijen) ──
  // Drizzle-kit push --force kan handmatig aangemaakte UNIQUE-constraints als
  // "drift" droppen. apply-additive loopt ná push opnieuw om ze te herstellen.
  // Veiligheidscheck: dubbele rijen voorkomen dat de constraint wordt aangelegd;
  // in dat geval stopt het script met exit 1 en een duidelijke foutmelding —
  // nooit stil doorgaan met een schema zonder constraint.
  for (const cs of constraintStappen) {
    // 1. Al aanwezig? Dan niets te doen.
    const bestaatRes = await pool.query(
      "SELECT 1 FROM pg_constraint WHERE conname = $1",
      [cs.naam],
    );
    if (bestaatRes.rowCount > 0) {
      console.log(`[apply-additive] OK (reeds aanwezig): constraint ${cs.beschrijving}`);
      continue;
    }

    // 2. Controleer op dubbele rijen vóór we de constraint aanleggen.
    const kolommen = cs.kolommen.map((k) => `"${k}"`).join(", ");
    ...
    if (dubbelsRes.rowCount > 0) {
      console.error(
        `[apply-additive] FATALE FOUT: constraint "${cs.naam}" kan niet worden aangelegd — ` +
        `er bestaan dubbele combinaties in "${cs.tabel}". ` +
    ...
    // 3. Veilig om toe te voegen.
    try {
      await pool.query(
        `ALTER TABLE "${cs.tabel}"
           ADD CONSTRAINT "${cs.naam}"
           UNIQUE (${kolommen})`,
      );
      console.log(`[apply-additive] OK (aangelegd): constraint ${cs.beschrijving}`);
    } catch (err) {
      console.error(
        `[apply-additive] FATALE FOUT bij aanleggen constraint "${cs.naam}": ${err.message}`,
      );
      await pool.end();
      process.exit(1);
    }
