...
// Constraints die aanwezig moeten zijn maar bewust NIET in het drizzle-schema
// staan (uniqueIndex in drizzle gaf eerder deployment-validatiefouten; ze worden
// via apply-additive.mjs aangelegd). drizzle-kit push --force zou zo'n constraint
// als drift kunnen droppen — deze check maakt dat direct zichtbaar.
...
async function main() {
  ...
  for (const check of constraintChecks) {
    try {
      ...
      if (res.rowCount > 0) {
        console.log(`[schema-healthcheck] OK: constraint ${check.beschrijving}`);
      } else {
        console.error(
          `[schema-healthcheck] FOUT: constraint ontbreekt — ${check.beschrijving} (${check.naam}). Mogelijk gedropt door drizzle-kit push; apply-additive.mjs herstelt dit bij de volgende run.`,
        );
        fouten++;
      }
    } catch (err) {
      console.error(
        `[schema-healthcheck] FOUT bij constraint-check "${check.beschrijving}": ${err.message}`,
      );
      fouten++;
    }
  }
