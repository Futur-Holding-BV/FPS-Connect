...
- **Paginatitel-merkteken (aug 2026):** accentlijn is 4px; gekleurd merkteken vóór de paginatitel via CSS `[data-hoofdstuk] [data-paginatitel]::before` (+ `--hoofdstuk-actief` gezet in de layout). Opt-in per h1 — NOOIT een automatische h1-selector (geneste previews renderen eigen h1's); geportalde dialogen vallen buiten de scope. Communicatie-fullscreenroutes (/berichten, /werk-inbox) hebben een aparte wrapper zonder data-hoofdstuk → daar bewust geen merkteken.
