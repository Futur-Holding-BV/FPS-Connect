...
## Hoe toepassen
- Download-URL van een `/objects/…`-pad = ALTIJD `/api/storage/objects/<rest>` (segmentsgewijs ge-encodeerd). **`/api/storage/files?path=…` bestaat NIET als route** ("Cannot GET"), ook al gebruiken oudere modules (facturen/offertes/snagstream/mandagstaat) dat patroon nog — die links zijn vermoedelijk dood; check vóór hergebruik.
