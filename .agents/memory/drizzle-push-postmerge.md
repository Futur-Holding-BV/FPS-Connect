...
**Parallelle merges (18-08-2026):** meerdere taak-merges op één dag kunnen (1) hetzelfde migratienummer claimen (3× 0079) — hernummer de NIET-gedeployede bestanden naar vrije nummers én update `schema_migraties.naam` in de dev-DB; (2) de gegenereerde API-clients terugzetten naar een stale versie — na elke merge-reeks `codegen` + `typecheck:libs` + beide artifact-typechecks draaien vóór je iets anders doet. De rename-check vergelijkt met origin/main en bijt alleen op branches, niet op main-push-CI.
